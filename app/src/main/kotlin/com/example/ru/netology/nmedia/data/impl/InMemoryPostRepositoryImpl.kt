...
class InMemoryPostRepositoryImpl : PostRepository {
    ...
    override fun likeById(id: Long) {
        ...
        posts = posts.map {
            if (it.id.toLong() != id) it
            else it.copy(byLikedMe = !it.byLikedMe, likeAmount = if (!it.byLikedMe) it.likeAmount + 1
            else it.likeAmount - 1)
        }
        data.value = posts
    }
